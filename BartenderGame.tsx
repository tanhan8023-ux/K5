...
export function BartenderGame({ onBack, apiSettings, personas, messages, setMessages, userProfile, worldbook }: Props) {
  ...
  }, [selectedPersonaId, selectedIngredients, gameState, bartenderComment, question, userAnswer, history, userDice, aiDice, winner, intoxication, userIntoxication]);
  ...
  const handleUserDecision = async (choice: 'drink' | 'truth') => {
      ...
      if (choice === 'drink') {
          ...
          const text = await generateGameContent(`[系统提示：用户选择了“大冒险/喝酒”。请发挥你的创意，现场调制一杯名字古怪、成分离谱的“惩罚特饮”，描述它的外观和气味，并看着用户喝下去。]${drunkPrompt}`);
          if (typeof text === 'string') {
              setHistory(prev => [...prev, { role: 'bartender', text }]);
              syncToChat(`我选择喝酒。\n${selectedPersona?.name}: ${text}`, 'user');
          }
