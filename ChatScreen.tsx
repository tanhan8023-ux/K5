...
import { fetchAiResponse, generateMoment } from '../services/aiService';
...
const renderTextWithStickers = (text: string) => {
  if (!text) return null;
  const parts = text.split(/(\[STICKER:\s*[^\]]+\])/g);
  ...
  const handleSend = async (text: string, msgType: 'text' | 'transfer' | 'relativeCard' | 'sticker' | 'listenTogether' = 'text', amount?: number, transferNote?: string, relativeCard?: { limit: number; status: 'active' | 'cancelled' }, sticker?: string, theaterId?: string) => {
    ...
    debouncedAiResponseTimeout.current = setTimeout(async () => {
      ...
      try {
        ...
        setMessages(prev => [...prev, { id: Date.now().toString(), personaId: currentPersona.id, role: 'model', text: `错误: ${error.message}`, timestamp: new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false }), isRead: true, theaterId }]);
      } finally {
        pendingRequests.current = Math.max(0, pendingRequests.current - 1);
        setIsTyping(pendingRequests.current > 0);
      }
                      ...
                      >
                        ...
                        <div className={`flex flex-col gap-2 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                          {msg.text ? (
                            ...
                            >
                              {(() => {
                                const parts = msg.text.split(/(\[STICKER:\s*[^\]]+\])/g);
                                const textParts = parts.filter(p => !p.match(/\[STICKER:\s*[^\]]+\]/)).join('').trim();
                                const stickerParts = parts.filter(p => p.match(/\[STICKER:\s*[^\]]+\]/));
                                ...
                                return (
                                  <div className="flex flex-col gap-1">
                                    ...
                                    {stickerParts.map((part, i) => {
                                      ...
                                      if (content === 'image') {
                                         return <span key={i} className="text-xs text-neutral-400">[表情包图片]</span>;
                                      }

                                      let src = content;
                                      if (!content.startsWith('http') && !content.startsWith('data:')) {
                                         // Fallback for keywords if not a URL
                                         // But in ChatScreen we usually have URLs from userProfile or Dicebear
                                         // If it's just a keyword, treat as Dicebear seed
                                      ...
                                      return (
                                        <img 
                                          ...
                                          src={src} 
                                          alt="sticker" 
                                          className="w-32 h-32 object-contain rounded-lg bg-neutral-100/50 my-1" 
