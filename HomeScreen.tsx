...
interface Props {
  onNavigate: (screen: 'chat' | 'persona' | 'api' | 'theme' | 'music' | 'xhs' | 'treehole' | 'taobao' | 'fooddelivery' | 'bartender') => void;
  onLock: () => void;
  theme: ThemeSettings;
  unreadCount: number;
      ...
      >
            ...
            <div className="bg-white/60 backdrop-blur-xl rounded-[1.5rem] p-4 flex flex-col justify-center shadow-sm border border-white/50 relative overflow-hidden h-[130px]">
              ...
              <div className="relative z-10">
                ...
                <div className="text-[11px] text-neutral-400 mt-2">
                  {userProfile.anniversaryDate ? `起始日: ${userProfile.anniversaryDate}` : '请在接口与人设中设置纪念日'}
                </div>
              </div>
            </div>
        ...
        <div className="min-w-full w-full h-full snap-center px-5 flex flex-col shrink-0 content-start pt-4">
          <div className="grid grid-cols-4 gap-y-4 gap-x-2 place-items-center">
            <AppIcon id="game" icon={HeartPulse} label="调酒真心话" onClick={() => onNavigate('bartender')} theme={theme} />
